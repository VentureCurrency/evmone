...
constexpr auto STACK_HEIGHT_INCREASE_LIMIT = 0x03FF;
...
EOFValidationError validate_types(bytes_view container, const EOF1Header& header) noexcept
{
    ...
    {
        const auto [inputs, outputs, stack_height_increase] = header.get_type(container, i);
        ...
        if (stack_height_increase > STACK_HEIGHT_INCREASE_LIMIT)
            return EOFValidationError::stack_height_increase_above_limit;
    }
...
///
/// Requires that the input is validated against truncation.
/// Returns computed stack height increase or the validation error.
    ...
    bytes_view code, size_t func_index, const EOF1Header& header, bytes_view container)
{
    ...
    for (size_t i = 0; i < code.size();)
    {
        ...
        if (opcode == OP_CALLF)
        {
            ...
            if (stack_height.max + callee_type.stack_height_increase > STACK_SIZE_LIMIT)
    ...
    const auto max_stack_height = max_stack_height_it->max;
    const auto stack_height_increase = max_stack_height - type.inputs;
    return stack_height_increase;
}
    ...
    while (!container_queue.empty())
    {
        ...
        while (!code_sections_queue.empty())
        {
            ...
            if (std::get<int32_t>(shi_or_error) !=
                header.get_type(container, code_idx).stack_height_increase)
                return EOFValidationError::invalid_stack_height_increase;
        }
...
std::string_view get_error_message(EOFValidationError err) noexcept
{
    ...
    case EOFValidationError::invalid_stack_height_increase:
        return "invalid_stack_height_increase";
    case EOFValidationError::stack_height_increase_above_limit:
        return "stack_height_increase_above_limit";
